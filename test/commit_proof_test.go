...
var (
	...
	k, n, d, space int64      = 8, 1024 * 1024, 64, 1024 * 1024
...
func TestGenCommitProofArgs(t *testing.T) {
	prover, err := pois.NewProver(k, n, d, []byte(minerId), space, 32)
	if err != nil {
		t.Fatal("create prover error", err)
	}
	...
	if err != nil {
		t.Fatal("generate idle file error", err)
	}
}

func TestRequestToGetChallenge(t *testing.T) {
	prover, err := pois.NewProver(k, n, d, []byte(minerId), space, 32)
	if err != nil {
		t.Fatal("create prover error", err)
	}
	...
	wg.Wait()
	t.Log("send request success")
}

func TestRequestToProveCommitProof(t *testing.T) {
	prover, err := pois.NewProver(k, n, d, []byte(minerId), space, 32)
	if err != nil {
		t.Fatal("create prover error", err)
	}
	...
	if err != nil {
		t.Fatal("send commits to tee worker error", err)
	}

	commitProof, accProof, err := prover.ProveCommitAndAcc(chals)
	if err != nil {
		t.Fatal("prove commit and acc proof error", err)
	}

	res, err := client.SendCommitProofs([]byte(minerId), commitProof, accProof)
	if err != nil {
		t.Fatal("send commit proof error", err)
	}
	t.Log("request to prove commit proof result", res)
}